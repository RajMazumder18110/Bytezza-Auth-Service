...
describe("Logout User", () => {
  ...
  describe("[200] - Successful logout", () => {
    it("Should return a 200 successful response.", async () => {
      const response = await usersApp.logout.$post(
        {},
        {
          headers: {
            Cookie: cookies,
    ...
    it("Should return a proper formatted json response.", async () => {
      ...
      expect(data.message).toStrictEqual("Successfully logged out!");
    });

    it("Should not contain set-cookie header.", async () => {
      const response = await usersApp.logout.$post(
        {},
        {
          headers: {
      ...
      expect(allCookies).toStrictEqual("ACCESS_TOKEN=; REFRESH_TOKEN=");
    });
  });
  ...
  describe("[401] - Cookies not available or invalid", () => {
    ...
    it("Should return 401 response when user trying to logout 2 times.", async () => {
      await usersApp.logout.$post(
        {},
        {
          headers: {
            Cookie: cookies,
